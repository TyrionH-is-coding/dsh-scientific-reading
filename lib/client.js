...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function renderLiterature() {
      ...
      function renderNavigationTable() {
        ...
        state.items.forEach(function (paper) {
          ...
          var selection = el('td'); var checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = selections.values().includes(paper.paper_id); checkbox.setAttribute('aria-label', '选择 ' + (paper.title || '无题名文献')); checkbox.addEventListener('change', function () { selections.toggle(paper.paper_id, checkbox.checked); renderBatchToolbar(); }); selection.appendChild(checkbox); tr.appendChild(selection);
    ...
    function buildSettingsCard() {
      ...
      var mineruNote = el('div', 'sr-dim', 'MinerU 精准解析使用宿主环境变量 MINERU_API_TOKEN；插件不保存密钥。设置后需重启 DSH，解析时 PDF 会上传至 MinerU 官方服务器。');
