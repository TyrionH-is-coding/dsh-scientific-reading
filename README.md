...
主线正在准备 rc.7：设置页改为模型与连接、PDF 解析、文献库、关于四组纵向设置行；翻译由服务端绑定原文和批次 SHA，保留已验证的部分，只补剩余块。本批初次提交后最多两次补试，随后暂停等待用户继续。rc.6 已整合六项个人记录、阅读成果与图表索引，以及工作簿排序和冲突保护。

已公开的 rc.5 修复 MinerU 空图表条目触发 `img_path 不能为空` 的问题。仅确认解析条目无可用内容时记录原始索引、页码及人工复核警告，保留原始 JSON 和来源哈希；缺图但仍有内容时报告完整性错误，不静默丢弃。空条目不能证明原 PDF 没有内容，仍须按报告核对对应页和相邻条目。
...
当前公开附件见 [GitHub Releases](https://github.com/TyrionH-is-coding/dsh-scientific-reading/releases)。新主线改动仍需按其源码提交和安装包单独验收；[早期 rc.1 发布修复记录](docs/releases/v0.1.0/RELEASE_GATE.md)仅作历史留存。源码目录和 GitHub 自动生成的 Source code 压缩包不能代替安装包。
