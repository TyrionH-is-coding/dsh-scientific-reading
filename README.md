...
重新启动同一 DSH Profile，新建会话并选择 **“文献模式”**。文献工具、文献页和“设置与状态”页会在该模式下启用。完整的校验、启动、更新和卸载步骤见[使用指南](docs/getting-started.md)。

### 2. 配置模型与 MinerU

| 配置 | 在哪里设置 | 用途 |
| --- | --- | --- |
| **模型服务** | DSH 原生设置中的模型配置及会话模型选择器 | 摘要翻译、全文翻译与阅读讨论 |
| **MinerU 免费 API Key** | 插件“设置与状态”中的 PDF 解析设置 | 将 PDF 解析为正文、图表与公式 |
