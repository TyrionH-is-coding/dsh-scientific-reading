...
def _sheet(workbook, name, columns, rows, table_name, widths):
    ...
    for index, header in enumerate(columns, 1):
        ...
        if header in {"文献 ID", "行标识", "记录 ID", "资产 ID"}:
...
def write_workbook(service, path, papers, records, assets):
    ...
    for paper in papers:
        key = uuid.uuid4().hex
        paper["行标识"] = key
        baseline[key] = {"paper_id": paper["文献 ID"], "fields": {field: paper[label] for label, field in USER_FIELDS.items()}}
        rows.append(tuple(paper.get(header, "") for header in XLSX_COLUMNS))
    ...
    sheet = _sheet(workbook, "文献", XLSX_COLUMNS, rows, "Literature", widths)
    ...
    for row, paper in enumerate(papers, 2):
        ...
        for name in ("PDF", "Reader"):
            _link(service, sheet.cell(row, XLSX_COLUMNS.index(name) + 1), paper_id, paper[name], "打开")
        for name, columns, counts in (("阅读成果", REVIEW_COLUMNS, count_records), ("图表索引", ASSET_COLUMNS, count_assets)):
            cell = sheet.cell(row, XLSX_COLUMNS.index(name) + 1)
            if counts[paper_id]:
                _jump(cell, paper_id, name, columns, f"查看 {counts[paper_id]} 项")
            else:
                _text(cell, "未生成")
        cell = sheet.cell(row, XLSX_COLUMNS.index("文献链接") + 1)
        if isinstance(cell.value, str) and cell.value.startswith(("https://", "http://")):
            cell.hyperlink = cell.value
    for row, record in enumerate(records, 2):
        paper_id = record["文献 ID"]
        _jump(reading.cell(row, 1), paper_id, "文献", XLSX_COLUMNS, record["文献名"].replace('"', '""'))
    ...
    for row, asset in enumerate(assets, 2):
        ...
        _jump(figures.cell(row, 1), paper_id, "文献", XLSX_COLUMNS, title.replace('"', '""'))
    ...
    validation.add(f"C2:C{max(2, sheet.max_row)}")
    sheet.column_dimensions.group("R", "Y", hidden=True)
    instructions = workbook.create_sheet("说明")
    for line in (
        "Deep Literature · 文献管理", "黄色六列可编辑；其余信息由系统维护，修改后不会回写。",
    ...
    sync.append(("contract", "xlsx-library-v2"))
    ...
    with sqlite3.connect(service.data_root / "library.sqlite") as conn:
        conn.execute("INSERT INTO xlsx_exports VALUES(?,?,?)", (export_id, stamp, json.dumps(baseline, ensure_ascii=False)))
