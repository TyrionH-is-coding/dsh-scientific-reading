...
- 生产生成器提交：`2ee0f86`（完整 SHA 以引擎仓库 Git 记录为准）。
- 直接 DOM：左栏导读位于目录之前，移动端在生成期输出独立抽屉内容；未使用 `toolbar.innerHTML` 或运行时导读搬移。
- 构建身份：`reader-html-v2.2-periodical`。
- 定向测试：reader、renderer、service 共 67 项通过。
- 引擎全量：852 项通过、3 项跳过；飞书凭证为空，前后台均显式加载隔离 worktree。
- 浏览器验收：1440×900、1280×720、900×720、390×844 均无横向溢出；语言、三态阅读、导读定位、目录折叠、图表弹窗和焦点归还通过。
- 外部状态：未写飞书、未触发机构认证、未注入 3080 Persistent Profile、未推送 GitHub。
