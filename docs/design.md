...
插件提供独立【设置与状态】页。首次安装只自动打开一次；以后即使环境未完全配置也不反复强制跳转。页面首次展示只读取缓存和执行无副作用的本机静态检查，机构会话、MinerU API 和本机 MinerU 均由用户点击【重新检测】后验证。

页面集中展示机构与专用 Chrome 会话、本机/API MinerU、CloakBrowser 可选增强包、数据根、SQLite、Excel、磁盘空间和运行版本。普通设置项与健康状态分开，页面不承担论文任务控制。
