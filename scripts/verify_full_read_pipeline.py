...
def _populate_mineru(generation, metadata, source_sha: str) -> None:
    ...
    for name in ("source_map.json", "parse_report.json"):
        ...
        if name == "parse_report.json":
            ...
            payload["provider_version"] = "fixture-1"
...
class AcceptanceMineruService:
    def run(self, _root, metadata, _method, **kwargs):
...
def _engine_root() -> Path:
    ...
    if value:
        ...
        root = (Path(__file__).resolve().parents[1] / "engine").resolve()
...
def make_pipeline(data_root):
    pipeline=ReadingPipeline(data_root,pdf_provider=Provider(root/"fixture.pdf"),mineru_service=Mineru(),full_read_service=Full(),reader_renderer=FullReadRenderer(publish_hook=staged))
...
def _formal_scenario(root: Path, engine: Path, interruption: str) -> dict:
    ...
    scenario_deadline = time.monotonic() + 45
    ...
    while True:
        if time.monotonic() >= scenario_deadline:
            raise RuntimeError("formal_scenario_timeout")
...
def verify() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="sr7-") as temporary:
        ...
        expected_counts = {"ensure_pdf", "parse_mineru", "translation:batch-0001", "translation:batch-0002", "render_reader", "schedule_derived_updates", "export"}
        ...
        if not counts_complete:
            raise RuntimeError("formal_stage_count_mismatch")
