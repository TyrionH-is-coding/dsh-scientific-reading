...
assert.match(source, /exports.inject = \['slots', 'sessions', 'sidebarRight', 'sidebarRightTabs'\]/, '工作区使用宿主会话与右侧栏服务')
assert.match(source, /ctx.sidebarRightTabs.register/, '文献和设置必须注册为原生侧栏页面')
assert.match(source, /name:'sidebar.right.pane.tab'/, '侧栏页面使用宿主内容插槽')
assert.match(source, /conversation.session.header.utilities/, '会话入口使用可扩展的 utilities 插槽')
assert.doesNotMatch(source, /conversation.session.header.corner/, '不得覆盖宿主唯一的侧栏展开控件')
assert.match(source, /name:'shell.overlay'/, '空白会话也必须能打开文献工作区')
assert.doesNotMatch(source, /sidebar.footer.action|sr-workbench-dialog|name: 'conversation.view'/, '不再保留左下角入口、工作区弹窗或重复页签')
...
assert.match(source, /var mountController = React\.useMemo[^]*renderLiterature\(ctx\.sessions,[^]*ref: mountController\.ref/, '模式视图必须复用稳定 callback ref 并注入 sessions')
