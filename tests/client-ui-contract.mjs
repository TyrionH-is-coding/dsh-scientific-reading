...
assert.match(source, /exports.inject = \['slots', 'connection', 'remote', 'sessions'\]/, 'client 必须注入 sessions 才能按会话 preset 开关标签')
...
assert.match(source, /var mountController = React\.useMemo[^]*renderLiterature\(ctx\.sessions, openNativeModels\)[^]*ref: mountController\.ref/, '模式视图必须复用稳定 callback ref 并注入 sessions')
